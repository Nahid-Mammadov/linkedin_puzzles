(function startLinkedInPuzzleRequests() {
  ...
  if (globalThis.__linkedinRequestSolverLoaded) {
    if (typeof globalThis.__linkedinRequestSolverLoaded === "function")
      globalThis.__linkedinRequestSolverLoaded();
    return;
  }
  globalThis.__linkedinRequestSolverLoaded = true;
  if (
    window.top === window &&
    document.querySelector("iframe[src*='/games/view/']")
  )
    return;
    ...
    for (let index = 0; index < documents.length && index < 8; index++) {
      for (const frame of documents[index].querySelectorAll("iframe")) {
        try {
          ...
          /* Cross-origin frames are not game data sources. */
        }
      }
    }
    return documents;
  ...
  function completed() {
    return (
      /\/results\/?$/.test(location.pathname) ||
      gameDocuments().some((doc) =>
        [...doc.querySelectorAll("a,button")].some(
          (el) =>
            !el.closest("#linkedin-logic-solver") &&
            (el.textContent || "").trim() === "See results",
        ),
      )
    );
  }
  ...
  async function message(type, extra = {}) {
    ...
    const timeout = new Promise((_, reject) => {
      timer = setTimeout(
        () =>
          reject(
            new Error(
              "The extension is taking too long to respond. Reload the extension and this tab, then retry.",
            ),
          ),
        10000,
      );
    ...
    if (!response?.ok)
      throw new Error(
        response?.error || "The extension could not read this game's data.",
      );
    return response;
  }
  ...
  function assertPage(url) {
    if (location.href !== url)
      throw new Error("Request cancelled because the puzzle page changed.");
  }
  ...
  function localSources() {
    const retained =
      globalThis.LinkedInPuzzleBootstrap?.captureVisible?.() || [];
    const live = gameDocuments()
      .flatMap((doc) => [...doc.querySelectorAll("script,code")])
  ...
  async function prepareVoyager(game, url) {
    ...
    for (let attempt = 0; Date.now() < deadline; attempt += 1) {
      ...
      if (
        sources.length === previousSources.length &&
        sources.every((source, i) => source === previousSources[i])
      ) {
        await delay(100, url);
        continue;
      }
      ...
      try {
        const state =
          game === "pinpoint"
            ? {
                blueprintGameState: [
                  parsers.parsePinpointSolutions(sources)[0],
                ],
              }
            : game === "crossclimb"
        ...
        if (!urn)
          throw new Error(
            "LinkedIn has not supplied this game's save identifier.",
          );
        return { state, urn };
      } catch (error) {
        lastError = error;
      ...
      if (solving)
        setStatus("Waiting for LinkedIn to load the puzzle…", "working");
      await delay(100, url);
    }
    throw (
      lastError ||
      new Error(
        "LinkedIn has not loaded the puzzle data. Open the board and retry.",
      )
    );
  }
  ...
  function csrfToken() {
    ...
    if (!token)
      throw new Error("Sign in to LinkedIn to save a completed game.");
    return token;
  }
  ...
  async function sendSave(endpoint, body, headers, url) {
    ...
    assertPage(url);
    if (!response.ok)
      throw new Error(`LinkedIn rejected the save: HTTP ${response.status}.`);
    return response;
  }
  ...
  async function submitVoyager(game, url, startedAt) {
    ...
    if (!queryId)
      throw new Error("LinkedIn's save-query identifier is unavailable.");
    if (game !== "pinpoint")
      await delay(Math.max(0, 1000 - (Date.now() - startedAt)), url);
    const body = requests.voyagerSave(
      game,
      urn,
      queryId,
      state,
  ...
  async function submitSdui(game, url, startedAt) {
    ...
    if (!context?.game)
      throw new Error(
        "LinkedIn's puzzle data is unavailable. Open the game board and retry.",
      );
    if (!context.template) {
      const error = new Error(
        "LinkedIn's initial save request was not captured, even after reloading. No save was sent.",
      );
      error.code = "MISSING_SAVE_CONTRACT";
      throw error;
    }
    await delay(Math.max(0, 1000 - (Date.now() - startedAt)), url);
    const body = requests[game + "Save"](
      context.game,
      context.template,
      Date.now() - startedAt,
    );
    const response = await sendSave(
      ...
      body,
      { "Content-Type": "application/json", "x-li-rsc-stream": "true" },
      url,
    );
    const path = requests.sduiResponse(await response.text());
    if (path !== `/games/${game}/results/`)
      throw new Error("Save response belongs to another game.");
  }

  async function solve(recovered = false) {
    if (solving) return;
    if (completed()) {
      setStatus("This game is already completed.", "success");
      return;
    }
    ...
    try {
      ...
      sessionStorage.setItem(
        pendingKey,
        JSON.stringify({ game, path: location.pathname, savedAt: Date.now() }),
      );
      ...
      if (
        ...
        setStatus(
          "Recovering LinkedIn's save request. Reloading and resuming…",
          "working",
        );
        location.reload();
        return;
  ...
  async function verifyPending() {
    ...
    try {
      ...
      /* Ignore malformed page storage. */
    }
    if (!pending) return;
    sessionStorage.removeItem(pendingKey);
    if (
      pending.game !== currentGame() ||
      pending.path !== location.pathname ||
      Date.now() - pending.savedAt > 120000
    )
      return;
    const url = location.href;
    solving = true;
    ...
    try {
      for (let attempt = 0; attempt < 48; attempt += 1) {
        if (completed()) {
          setStatus("Solved by request. Verified after reload.", "success");
          return;
        }
        await delay(250, url);
      }
      throw new Error(
        "Save was not confirmed after reload. No UI fallback was attempted.",
      );
    } catch (error) {
  ...
  function updatePanel() {
    ...
    panel.querySelector(".lls__title").textContent =
      games[game] || "Puzzle Solver";
    if (!solving)
      setStatus(
        completed()
          ? "This game is already completed."
          : "Ready to solve by request.",
      );
  ...
  panel
    .querySelector(".lls__diagnostics button")
    .addEventListener("click", async () => {
      const report = panel.querySelector("pre");
      try {
        report.textContent = JSON.stringify(
          (await message("lls-debug-requests", { all: true })).requests,
          null,
          2,
        );
      } catch (error) {
  ...
  chrome.runtime.onMessage?.addListener((message, sender, respond) => {
    if (
      ...
      !/^lls-service-(state|solve)$/.test(message?.type || "")
    )
      return;
    const game = currentGame();
    ...
    respond({
      ...
      solving,
      completed: completed(),
      phase: status.dataset.state || "idle",
  ...
  let recovery;
  try {
    recovery = JSON.parse(sessionStorage.getItem(recoveryKey));
  } catch {
    /* Ignore malformed page storage. */
  }
  sessionStorage.removeItem(recoveryKey);
  if (
    recovery?.game === currentGame() &&
    recovery.path === location.pathname &&
    Date.now() - recovery.savedAt >= 0 &&
    Date.now() - recovery.savedAt < 120000
  ) {
    void solve(true);
  } else {
    void verifyPending();
