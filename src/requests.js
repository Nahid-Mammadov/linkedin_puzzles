(function (root) {
  ...
  function queensSave(game, template, elapsedMs) {
    ...
    if (!puzzle || identity?.gameTypeId !== "3")
      throw new Error("Queens puzzle data is unavailable.");
  ...
  function tangoSave(game, template, elapsedMs) {
    ...
    return completedSave(
      game,
      template,
      "tangoBoardStateBinding",
      puzzle.solution.map((value) => value.replace("LotkaCellValue_", "")),
      elapsedMs,
    );
  }
  ...
  function zipSave(game, template, elapsedMs) {
    ...
    if (
      game?.gameUrn?.gameTypeId !== "6" ||
      !Number.isInteger(total) ||
      total < 1 ||
      puzzle.solution?.length !== total ||
      new Set(puzzle.solution).size !== total ||
      puzzle.solution.some(
  ...
  function patchesSave(game, template, elapsedMs) {
    ...
    if (
      game?.gameUrn?.gameTypeId !== "8" ||
      !Number.isInteger(total) ||
      total < 1 ||
      !cells ||
      cells.length !== total ||
      new Set(cells).size !== total ||
    ...
    return completedSave(
      game,
      template,
      "patchesBoardStateBinding",
      puzzle.solution.map((region) => region.cellIdxes.join("_")),
      elapsedMs,
    );
  }
  ...
  function wendSave(game, template, elapsedMs) {
    ...
    if (
      game?.gameUrn?.gameTypeId !== "4" ||
      !letters ||
      !cells ||
      cells.length !== letters.filter(Boolean).length ||
    ...
    return completedSave(
      game,
      template,
      "wendBoardStateBinding",
      paths.map((path) => path.join("_")),
      elapsedMs,
    );
  }
  ...
  function completedSave(game, template, boardKey, board, elapsedMs) {
    ...
    if (
      body.requestId !== "updateGameState" ||
    ...
    for (const states of [body.states, body.requestedArguments.states]) {
      if (!Array.isArray(states))
        throw new Error("Captured request has no state bindings.");
      const values = new Map(states.map((entry) => [entry.key, entry]));
      if (
        String(values.get("gameBoardPuzzleId")?.value?.value) !==
        String(game.gameUrn.puzzleId)
      ) {
      ...
      if (Number.isFinite(elapsedMs)) {
        ...
        if (!time?.value || time.value.type !== "bigint")
          throw new Error("Missing elapsed-time binding.");
        time.value.value = String(Math.max(1000, Math.round(elapsedMs)));
      }
    }
    return body;
  }
  ...
  function sduiResponse(text) {
    for (const line of text.split("\n")) {
      ...
      if (payload.response.errors.length)
        throw new Error("LinkedIn rejected the game save.");
      const actions = payload.response.completionAction?.actions || [];
      const resultsPath = actions
        .map((action) => action.value?.content?.url?.urlValue?.url)
        .find((url) => /^\/games\/[a-z-]+\/results\/?$/.test(url));
      if (!resultsPath)
        throw new Error("LinkedIn did not confirm game completion.");
      return resultsPath;
    }
  ...
  function crossclimbState(rungs) {
    const ordered = rungs
      .slice()
      .sort((a, b) => a.solutionRungIndex - b.solutionRungIndex);
    if (
      ordered.length < 3 ||
      ordered.some(
        (rung, index) => rung.solutionRungIndex !== index || !rung.word,
      )
    ) {
      throw new Error("Crossclimb delivered an incomplete ladder.");
    }
    return {
      crossClimbGameState: ordered.map((rung) => ({
  ...
  function sudokuState(puzzle) {
    ...
    if (
      !Number.isInteger(size) ||
      puzzle.gridColSize !== size ||
      puzzle.solution?.length !== size * size ||
      !Array.isArray(puzzle.presetCellIdxes) ||
      puzzle.solution.some(
        (value) => !Number.isInteger(value) || value < 1 || value > size,
      )
    ) {
      throw new Error("Mini Sudoku delivered an incomplete solution.");
    }
    return {
      miniSudokuGameState: puzzle.solution.flatMap((value, index) =>
        puzzle.presetCellIdxes.includes(index)
          ? []
          : [{ cellIdx: index, cellContentUnion: { cellValue: value } }],
      ),
    };
  }
  ...
  function voyagerSave(game, urn, queryId, state, elapsedMs) {
    ...
    if (!ids[game] || gameUrn([urn], ids[game]) !== urn || !queryId)
      throw new Error("Invalid game save identity.");
    const record = {
    ...
    if (game !== "pinpoint")
      Object.assign(record, {
        timeElapsed: Math.max(1, Math.round(elapsedMs / 1000)),
        isFlawless: true,
        completionAttributes: { isHintFree: true, isMistakeFree: true },
      });
    return {
      variables: {
        entity: { entity: { gameStoredRecord: record }, resourceKey: urn },
      },
      queryId,
